...
import * as jpl from '@jplorg/jpl';
import pkg from '@jplorg/jpl/package.json' with { type: 'json' };
import fs from 'fs';
import os from 'os';
...
let multilineInput: string | null;

const homeDir = os.homedir();
const historyFile = homeDir
  ? path.join(homeDir, '.jpl_repl_history')
  : undefined;
...
if (historyFile) {
  rl.on('history', (history) => {
    try {
      fs.writeFileSync(
        historyFile,
        history.filter(Boolean).reverse().join('\n') + '\n',
        {
          mode: 0o600,
        },
      );
...
async function handle(input: string): Promise<void> {
  ...
  if (replKeys.some((replKey) => t.startsWith(replKey))) {
    ...
    switch (command) {
      ...
      case 'i':
        ...
        try {
          ...
          if (
            jpl.JPLSyntaxError.is(err) &&
            (err as jpl.JPLSyntaxError).at >=
              (err as jpl.JPLSyntaxError).src.length
          ) {
          ...
          if (jpl.JPLSyntaxError.is(err))
            console.log(
              `${(err as jpl.JPLSyntaxError).name ?? 'JPLError'}: ${(err as jpl.JPLSyntaxError).message}`,
            );
          else console.log((err as Error).stack);
        }
        break;
      ...
      default:
        console.log(
          `Error: unrecognized REPL command ${defaultReplKey}${command}\n`,
        );
        printHelp();
    }
  } else {
    ...
    try {
      ...
      console.log(
        inputs.map((output) => JSON.stringify(output, null, 2)).join(', '),
      );
      if (measureTime) console.log(` -> took ${diff! / 1000}s`);
    } catch (err) {
      if (
        jpl.JPLSyntaxError.is(err) &&
        (err as jpl.JPLSyntaxError).at >= (err as jpl.JPLSyntaxError).src.length
      ) {
        // program is incomplete -> request additional input
        multilineInput = fullLine + '\n';
        rl.setPrompt(multilinePrompt);
...
function parseBool(
  ...
  if (b.length === 0) v = defaultValue;
    ...
    b === 'on' ||
    ['true', 'yes', 'enabled'].some((e) => e.startsWith(b))
  )
    v = true;
  else if (
    b === 'off' ||
    ['false', 'no', 'disabled'].some((e) => e.startsWith(b))
  )
    v = false;
  if (typeof v === 'boolean') {
...
function printBool(value: boolean): string {
...
function printHelp(): void {
  const commands = [
    ...
    [
      't',
      printBool(measureTime),
      'Set whether execution time should be measured',
    ],
    ['q', '', 'Exit the REPL'],
  ];
  ...
  commands.forEach(([c, a, d]) =>
    console.log(
      `${defaultReplKey}${c} ${a}${' '.repeat(aLen - a.length + 3)}${d}`,
    ),
  );
