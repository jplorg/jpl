...
/** Parse access at i */
export async function parseAccess(
  src: string,
  i: number,
  c: ParserContext,
  { identity }: AccessOptions = {},
): Promise<{
  i: number;
  is: boolean;
  ...
  for (;;) {
    ...
    if (!isIdentity && m.is) {
      ...
      selectors.push({
        op: OPA_FIELD,
        params: {
          pipe: [{ op: OP_STRING, params: { string: name } }],
          optional,
        },
      });
      continue;
    ...
    if (m.is) {
      ...
      if (m.is) {
        ...
        if (m.is) {
          ...
          selectors.push({
            op: OPA_SLICE,
            params: {
              from: [{ op: OP_CONSTANT_NULL, params: {} }],
              to: [{ op: OP_CONSTANT_NULL, params: {} }],
              optional,
            },
          });
          continue;
      ...
      selectors.push({
        op: OPA_SLICE,
        params: { from: opsLeft, to: opsRight, optional },
      });
      continue;
    }
...
export async function parseAssignment(
  ...
  { i: number; is: false } | { i: number; is: true; assignment: JPLAssignment }
> {
  let n = i;
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_SET, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_UPDATE, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_ADDITION, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_SUBTRACTION, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_MULTIPLICATION, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_DIVISION, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_REMAINDER, params: { pipe: ops } },
    };
  }
  ...
  if (m.is) {
    ...
    return {
      i: n,
      is: true,
      assignment: { op: OPU_NULL_COALESCENCE, params: { pipe: ops } },
    };
  }
...
export function parseNumber(
  ...
  i: number,
  c: ParserContext,
): { i: number; is: false } | { i: number; is: true; ops: Pipe } {
  let n = i;
  let is = false;
  let value = '';
...
export async function parseString(
  ...
  i: number,
  c: ParserContext,
): Promise<{ i: number; is: false } | { i: number; is: true; ops: Pipe }> {
  let n = i;
  let value = '';
  ...
  const interpolations: JPLInterpolation[] = [];
  for (;;) {
          ...
          for (let j = 0; j < 4; j += 1) {
            ...
            if (!m.is)
              return errorUnexpectedToken(src, n, c, {
                operator: 'string',
                message:
                  'incomplete unicode escape sequence: expected hex digit',
              });
            hexVal += (m as ReturnType<typeof hex>).value;
          }
    ...
    if (src.charCodeAt(n) < 0x20)
      return errorUnexpectedToken(src, n, c, { operator: 'string' });
  ...
  if (interpolations.length === 0)
    return {
      i: n,
      is: true,
      ops: [{ op: OP_STRING, params: { string: value } }],
    };
  ...
  return {
    ...
    is: true,
    ops: [
      { op: OP_INTERPOLATED_STRING, params: { interpolations, after: value } },
    ],
  };
}

/** Parse pipe at i */
export async function opPipe(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
/** Parse subpipe at i */
export async function opSubPipe(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
/** Parse subroute at i */
export async function opSubRoute(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opOutputConcat(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opTry(
  ...
  } else opsCatch = [{ op: OP_VOID, params: {} }];

  return {
    i: n,
    ops: [{ op: OP_TRY, params: { try: opsTry, catch: opsCatch } }],
  };
}

/** Parse or at i */
export async function opOr(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
/** Parse equality at i */
export async function opEquality(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  const comparisons: JPLComparison[] = [];
  for (;;) {
    let m = matchWord(src, n, c, { phrase: '==' });
    if (m.is) {
      ({ i: n } = m);
  ...
  return {
    i: n,
    ops: [{ op: OP_COMPARISON, params: { pipe: ops, comparisons } }],
  };
}

/** Parse comparison at i */
export async function opComparison(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opNot(
  ...
  return { i: n, ops: [...ops, { op: OP_NOT, params: {} }] };
}

/** Parse error suppression at i */
export async function opErrorSuppression(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  return {
    ...
    ops: [
      {
        op: OP_TRY,
        params: { try: result.ops, catch: [{ op: OP_VOID, params: {} }] },
      },
    ],
  };
}
...
export async function opDifference(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  return {
    i: n,
    ops: [{ op: OP_CALCULATION, params: { pipe: ops, operations } }],
  };
}

/** Parse multiplication at i */
export async function opMultiplication(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opNullCoalescence(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
/** Parse negation at i */
export async function opNegation(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  return { i: n, ops: [...ops, { op: OP_NEGATION, params: {} }] };
}

/** Parse if at i */
export async function opIf(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
/** Parse constant at i */
export function opConstant(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> | { i: number; ops: Pipe } {
  let n = i;

  let m = matchWord(src, n, c, { phrase: 'true', spaceAfter: true });
  if (m.is) {
    ({ i: n } = m);
    return { i: n, ops: [{ op: OP_CONSTANT_TRUE, params: {} }] };
  }

  m = matchWord(src, n, c, { phrase: 'false', spaceAfter: true });
  if (m.is) {
    ({ i: n } = m);
    return { i: n, ops: [{ op: OP_CONSTANT_FALSE, params: {} }] };
  }

  m = matchWord(src, n, c, { phrase: 'null', spaceAfter: true });
  if (m.is) {
    ({ i: n } = m);
    return { i: n, ops: [{ op: OP_CONSTANT_NULL, params: {} }] };
  }
...
/** Parse number at i */
export async function opNumber(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opNamedFunctionDefinition(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
    ...
    ops: [
      {
        ...
        params: {
          name,
          pipe: [
            { op: OP_FUNCTION_DEFINITION, params: { argNames, pipe: ops } },
          ],
        },
      },
    ],
...
export async function opFunctionDefinition(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  return {
    i: n,
    ops: [{ op: OP_FUNCTION_DEFINITION, params: { argNames, pipe: ops } }],
  };
}

/** Parse variable definition at i */
export async function opVariableAccess(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  if (!canAssign) {
    const ops: Pipe = [{ op: OP_VARIABLE, params: { name } }];
  ...
  let opAssignment: JPLAssignment;
  ...
  if (selectors.length === 0 && opAssignment.op === OPU_SET) {
    return {
      ...
      ops: [
        {
          op: OP_VARIABLE_DEFINITION,
          params: { name, pipe: opAssignment.params.pipe },
        },
      ],
    };
  }
...
export async function opValueAccess(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
  ...
  return {
    ...
    ops: [
      {
        op: OP_ASSIGNMENT,
        params: { pipe: ops, selectors, assignment: opAssignment },
      },
    ],
  };
}
...
export async function opObjectConstructor(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opArrayConstructor(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
...
export async function opStringLiteral(
  src: string,
  i: number,
  c: ParserContext,
): Promise<{ i: number; ops: Pipe }> {
