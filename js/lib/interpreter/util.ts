...
export function walkWhitespace(
  src: string,
  i: number,
  c: ParserContext,
): { i: number; is: boolean } {
...
export function match(
  ...
  i: number,
  c: ParserContext,
  { phrase, notBeforeSet, spaceBefore, spaceAfter }: MatchOptions,
): { i: number; is: boolean } {
  let n = i;
  ...
  if (!eot(src, n, c).is) {
    if (notBeforeSet && matchSet(src, n, c, { set: notBeforeSet }).is)
      return { i: n, is: false };

    if (
      spaceAfter &&
      !matchSet(src, n, c, { set: setVarAll, exclusive: true }).is
    )
      return { i: n, is: false };
  }
...
export function hex(
  ...
  i: number,
  c: ParserContext,
): { i: number; is: boolean; value: string } {
...
export function matchSet(
  ...
  i: number,
  c: ParserContext,
  { set, exclusive }: MatchSetOptions,
): { i: number; is: boolean; value: string } {
  const end = eot(src, i, c);
  if (end.is) return { i: end.i, is: false, value: '' };

  const char = src[i];
  if (set.includes(char) === !exclusive)
    return { i: i + 1, is: true, value: char };
...
export function safeVariable(
  src: string,
  i: number,
  c: ParserContext,
): { i: number; is: boolean; value: string; reserved?: boolean } {
...
export function whereIs(
  ...
  i: number,
  _c: ParserContext,
): { i: number; line: number; column: number } {
...
export function errorUnexpectedToken(
  src: string,
  i: number,
  c: ParserContext,
  { operator, message }: ErrorOptions = {},
): never {
  let errorMessage;
