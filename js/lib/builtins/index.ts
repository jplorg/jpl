...
const getBuiltins = createSingleton<Promise<Record<string, unknown>>>(
  async () => {
    await undefined;

    let result: Record<string, unknown> = { ...native };
    const program = await systemInterpreter.parse(builtins, {
      runtime: {
        vars: { internals, ...native },
        adjustResult: (_value, scope) => {
          const { internals: _, ...vars } = scope.vars;
          result = applyObject(result, Object.entries(vars)) as Record<
            string,
            unknown
          >;
          return [];
