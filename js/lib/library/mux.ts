...
export default function mux(
  ...
  let execCount = 1;
  const indices = args.map((arg): [number, number, unknown[]] => {
...
export function muxOne<T>(args: unknown[][], cb: (...args: any) => T): T[] {
  const argCount = args.length;
  ...
  if (execCount === 0) return [];
  const outputs = new Array<T>(execCount);
  const buffer = args.map(([v]) => v);
  let execIndex = 0;
  for (;;) {
...
export async function muxAll<T>(
  args: unknown[][],
  cb: (...args: any) => Promise<T[]> | T[],
): Promise<T[]> {
...
export function mergeSegments<T>(segments: T[][]): T[] {
  if (segments.length === 0) return segments as T[];
